            ...
            try
            {
                using (var doc = System.Text.Json.JsonDocument.Parse(jsonResponse))
                {
                             ...
                             endpointsObj.TryGetProperty("Endpoint", out itemsArray))
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine($"[APEX PARSE] Found Endpoints.Endpoint structure");
                    }
                    else
                    {
                    ...
                    foreach (var item in itemsArray.EnumerateArray())
                    {
                        ...
                        if (item.TryGetProperty("Sno", out var snoEl) || item.TryGetProperty("sno", out snoEl) || item.TryGetProperty("SNO", out snoEl))
                            endpoint.Sno = snoEl.TryGetInt32(out int sno) ? sno : 0;

                        if (item.TryGetProperty("Source", out var sourceEl) || item.TryGetProperty("source", out sourceEl) || item.TryGetProperty("SOURCE", out sourceEl))
                            endpoint.Source = sourceEl.GetString() ?? "";

                        if (item.TryGetProperty("IntegrationCode", out var codeEl) || item.TryGetProperty("integration_code", out codeEl) || item.TryGetProperty("INTEGRATION_CODE", out codeEl))
                            endpoint.IntegrationCode = codeEl.GetString() ?? "";

                        if (item.TryGetProperty("InstanceName", out var instEl) || item.TryGetProperty("instance_name", out instEl) || item.TryGetProperty("INSTANCE_NAME", out instEl))
                            endpoint.InstanceName = instEl.GetString() ?? "";

                        if (item.TryGetProperty("URL", out var urlEl) || item.TryGetProperty("base_url", out urlEl) || item.TryGetProperty("BASE_URL", out urlEl))
                            endpoint.BaseUrl = urlEl.GetString() ?? "";

                        if (item.TryGetProperty("Path", out var epEl) || item.TryGetProperty("endpoint", out epEl) || item.TryGetProperty("ENDPOINT", out epEl))
                            endpoint.Endpoint = epEl.GetString() ?? "";

                        if (item.TryGetProperty("Comments", out var commEl) || item.TryGetProperty("comments", out commEl) || item.TryGetProperty("COMMENTS", out commEl))
                            endpoint.Comments = commEl.GetString() ?? "";

                        if (item.TryGetProperty("apitype", out var apiTypeEl) || item.TryGetProperty("API_TYPE", out apiTypeEl) || item.TryGetProperty("api_type", out apiTypeEl) || item.TryGetProperty("ApiType", out apiTypeEl))
                            endpoint.ApiType = apiTypeEl.GetString() ?? "";

                        endpoints.Add(endpoint);
                        System.Diagnostics.Debug.WriteLine($"[APEX PARSE] Parsed: Sno={endpoint.Sno}, Source={endpoint.Source}, Code={endpoint.IntegrationCode}, ApiType={endpoint.ApiType}");
                    }
                }
            }
            catch (Exception ex)
